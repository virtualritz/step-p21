...
/// 215 explicit_attr = [attribute_decl] { `,` [attribute_decl] } `:` \[
...
pub fn entity_head(
    ...
    .map(|(_start, id, (constraint, subtype), _semicolon)| {
        (id, constraint, subtype)
    })
    .parse(input)
}
...
mod tests {
    ...
    fn entity_decl() {
        ...
        let (residual, (entity, _remark)) =
            super::entity_decl(exp_str).finish().unwrap();
    ...
    fn b_spline_curve() {
        ...
        let (residual, (entity, _remark)) =
            super::entity_decl(input).finish().unwrap();
        dbg!(&entity);
        assert_eq!(residual, "");
    }
