...
fn create_tree(
    mut head: Expression,
    tails: Vec<(BinaryOperator, Expression)>,
) -> Expression {
...
pub fn simple_factor(input: &str) -> ParseResult<'_, Expression> {
    let paren_expr =
        tuple((char('('), expression, char(')'))).map(|(_open, e, _close)| e);
    // ( \[ unary_op \] ( `(` expression `)` | primary ) )
    let unary = tuple((opt(unary_op), alt((paren_expr, primary)))).map(
        |(opt, expr)| {
            if let Some(op) = opt {
                Expression::Unary {
                    op,
...
/// 216 expression = [simple_expression] \[ [rel_op_extended]
/// [simple_expression] \] .
...
/// 243 interval = `{` [interval_low] [interval_op] [interval_item]
...
pub fn interval(input: &str) -> ParseResult<'_, Expression> {
    ...
    .map(|(_open, low, op_low, item, op_high, high, _close)| {
        Expression::Interval {
            op_low,
            op_high,
            low: Box::new(low),
...
pub fn query_expression(input: &str) -> ParseResult<'_, Expression> {
    ...
    .map(
        |(_start, _open, variable, _star, source, _bar, expr, _close)| {
            Expression::Query {
                variable,
...
mod tests {
    ...
    #[test]
    fn unary() {
        let (res, (expr, _remarks)) =
            super::expression("-1.0").finish().unwrap();
        assert_eq!(res, "");
        assert_eq!(
            expr,
            Expression::Unary {
    ...
    fn interval() {
        let (res, (expr, _remarks)) =
            super::interval("{1 <= date.day_component <= 31}")
                .finish()
                .unwrap();
        assert_eq!(res, "");
        dbg!(expr);
    ...
    fn binary() {
        let (res, (expr, _remarks)) =
            super::expression("1.0 + 2.0").finish().unwrap();
        assert_eq!(res, "");
        assert_eq!(
            expr,
            Expression::Binary {
    ...
    fn binary3() {
        let (res, (expr, _remarks)) =
            super::expression("1.0 + 2.0 - 3.0").finish().unwrap();
        assert_eq!(res, "");
        assert_eq!(
            expr,
            Expression::Binary {
    ...
    fn relation() {
        let (res, (expr, _remarks)) =
            super::expression("1 <= 2").finish().unwrap();
        assert_eq!(res, "");
    ...
    fn relation_self() {
        let (res, (expr, _remarks)) =
            super::expression("1 <= SELF").finish().unwrap();
        assert_eq!(res, "");
    ...
    fn binary_relation() {
        let (res, (expr, _remarks)) =
            super::expression("(1 <= SELF) AND (SELF <= 12)")
                .finish()
                .unwrap();
        assert_eq!(res, "");
        assert_eq!(
    ...
    fn qualifiable_factor() {
        let (res, (expr, _remarks)) =
            super::expression(r"x\group.attr").finish().unwrap();
        assert_eq!(res, "");
        assert_eq!(
            expr,
    ...
    fn entity_constructor() {
        let (residual, (ctor, _remarks)) =
            super::entity_constructor("point(0.0, 0.0, 0.0)")
                .finish()
                .unwrap();
        assert_eq!(residual, "");
    ...
    #[test]
    fn call_attr() {
        let (residual, (expr, _remarks)) =
            super::expression("f(a, b).attr").finish().unwrap();
        dbg!(expr);
        assert_eq!(residual, "");
    }
}
