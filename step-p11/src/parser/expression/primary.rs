...
pub fn primary(input: &str) -> ParseResult<'_, Expression> {
    alt((
        ...
        tuple((qualifiable_factor, many0(qualifier))).map(
            |(factor, qualifiers)| Expression::QualifiableFactor {
                factor,
                qualifiers,
            },
        ),
...
pub fn qualifiable_factor(input: &str) -> ParseResult<'_, QualifiableFactor> {
    alt((
        function_call,
        alt((attribute_ref, general_ref, population))
            .map(QualifiableFactor::Reference),
        constant_factor,
    ))
    .parse(input)
}

/// 999 function_call = ( [built_in_function] | [function_ref] )
...
/// ```
///
/// This function always requires `actual_parameter_list` in order to identify
...
pub fn built_in_function(input: &str) -> ParseResult<'_, BuiltInFunction> {
    // alt impl is up to 11-element tuple. In reverse order to match longer case
    // first.
    alt((
        alt((
...
mod tests {
    ...
    fn simple() {
        let (res, (q, _remarks)) =
            super::primary(r"x\group.attr").finish().unwrap();
        assert_eq!(res, "");
        if let Expression::QualifiableFactor { factor, qualifiers } = q {
            match factor {
                QualifiableFactor::Reference(name) => {
        ...
        if let Expression::QualifiableFactor { factor, qualifiers } = q {
            match factor {
                QualifiableFactor::FunctionCall { name, args } => {
                    assert_eq!(
                        name,
                        FunctionCallName::Reference("f".to_string())
                    );
    ...
    fn function_call_builtin() {
        let (res, (q, _remarks)) =
            super::primary("VALUE_UNIQUE(x)").finish().unwrap();
        assert_eq!(res, "");
        dbg!(q);
    }
    ...
    fn call_attr() {
        let (residual, (expr, _remarks)) =
            super::primary("f(a, b).attr").finish().unwrap();
        dbg!(expr);
        assert_eq!(residual, "");
    }
}
