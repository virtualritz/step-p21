...
pub fn aggregate_initializer(input: &str) -> ParseResult<'_, Expression> {
    ...
    .map(
        |(_open, elements, _close)| Expression::AggregateInitializer {
            elements,
        },
    )
    .parse(input)
...
mod tests {
    ...
    fn aggregate_initializer() {
        let (res, (expr, _remarks)) =
            super::expression("[1, 3, 6, 9*8, -12]").finish().unwrap();
        assert_eq!(res, "");
        assert_eq!(
            expr,
                ...
                elements: vec![
                    ...
                    Element {
                        expr: Expression::Binary {
                            ...
                            arg1: Box::new(Expression::Literal(Literal::Real(
                                9.0
                            ))),
                            arg2: Box::new(Expression::Literal(Literal::Real(
                                8.0
                            )))
                        },
                        repetition: None,
                    ...
                    Element {
                        expr: Expression::Unary {
                            ...
                            arg: Box::new(Expression::Literal(Literal::Real(
                                12.0
                            ))),
                        },
                        repetition: None,
