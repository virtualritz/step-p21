...
pub fn select_list(input: &str) -> ParseResult<'_, Vec<String>> {
...
pub fn select_extension(input: &str) -> ParseResult<'_, (String, Vec<String>)> {
...
mod tests {
    ...
    fn select() {
        let (res, (s, _remarks)) =
            super::select_type("SELECT (a, b)").finish().unwrap();
        assert_eq!(res, "");
        if let Type::Select {
            extensibility,
            types,
