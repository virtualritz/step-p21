//! Intermediate Representation ([IR]) legalized (semantically analyzed) from
//! [SyntaxTree]
//!
...
//! This step also introduces [Scope] and [Path] to represent scopes and names
//! in EXPRESS schema. Using [Namespace], we can look up corresponding [Path] to
//! a local identifier appears in a [Scope].
//!
//! ```text
...
//! ```
//!
//! This means `employee` has a field `pay` in addition to `name` inherited from
//! `person`, and an `employee` can be instantiated as a `person`.
//! Instances which contains two or more entity value are called "complex entity
//! instance", and they are mapped into exchange structures using one of two
//! rules, "internal mapping" or "external mapping":
//!
//! ```text
//! /* internal mapping */
...
//! and `#3` (`#4`) described by external mapping are same value of `employee`
//! (`student`) entity, but internal mapping cannot describe `#5` case.
//! Different from usual Object-Oriented Programming (OOP) languages like C++ or
//! Python, `person` can be both `employee` and `student` simultaneously,
//! i.e. a `person` object may have both `pay` field and `school_name` field
//! like as `#5`.
//!
//! This type of inheritance is called `ANDOR` in EXPRESS,
//! and it is the default constraint for supertype.
//! We can write this constraint explicitly in the entity declaration of
//! `person`:
//!
//! ```text
...
//! to look up subtype paths from supertype path before legalizing AST of
//! entities.
//!
...
//! This is represented by `ONEOF` constraint in `SUBTYPE_CONSTRAINT`
//! declaration. Second is `AND` constraint:
//!
//! ```text
//! ENTITY person;
//! END_ENTITY;
...
//! following rules:
//!
//! - Code generation only looks IRs, never looks AST. Every information
//!   required for code generation must be contained in IR.
//! - Code generation does not execute global analysis, e.g. check if a type
//!   reference refers a primitive type or not.
//!
