    ...
    fn legalize(
        ...
        let supertypes = if let Some(supertypes) = &entity.subtype_of {
            supertypes
                ...
                .iter()
                .map(|sup| {
                    TypeRef::from_path(ns, ss, &ns.resolve(scope, sup)?.0)
                })
        ...
        let constraints =
            if let Some(instantiables) = ss.instantiables.get(&path) {
                instantiables
                    .iter()
...
mod tests {
    ...
    fn legalize() {
        ...
        let scope =
            Scope::root().pushed(ScopeType::Schema, &example.schemas[0].name);
