...
//! guard was too narrow: an EXPRESS `simple_id` continues with a letter, a
//! digit or an underscore, so `FALSE_POSITIVE`, `TRUE_NORTH`, `TRUE2` and
//! `UNKNOWN_9` still failed to parse -- and those are ordinary names to find in
//! a schema.

use step_p11::ast::SyntaxTree;
