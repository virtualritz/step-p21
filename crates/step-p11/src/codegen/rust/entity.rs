...
fn use_place_holder(ty: &TypeRef) -> bool {
    match ty {
        ...
        TypeRef::Set { base, .. } | TypeRef::List { base, .. } => {
            use_place_holder(base)
        }
        _ => true,
    }
}
...
impl Entity {
    ...
    fn generate_any_enum(&self, tokens: &mut TokenStream) {
        ...
        let mut fields =
            vec![format_ident!("{}", self.name.as_str().into_safe())];
        let mut variants =
            vec![format_ident!("{}", self.name.to_pascal_case())];
        let mut constraints =
            vec![format_ident!("{}", self.name.to_pascal_case())];
        ...
        for ty in &self.constraints {
            match ty {
                TypeRef::Entity {
                    ...
                    if *is_supertype {
                        constraints.push(format_ident!(
                            "{}Any",
                            name.to_pascal_case()
                        ));
                    } else {
                        constraints
                            .push(format_ident!("{}", name.to_pascal_case()));
                    }
                }
                _ => unreachable!(),
            }
    ...
    /// Generate `impl AsRef<Self> for SelfAny` and `impl AsRef<Super> for
    /// SelfAny`
