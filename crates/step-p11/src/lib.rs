...
//! └──┬───────────────────────────────┘ │
//!    │ Code Generation                 │ step_p11's responsibility
//! ┌──▼────────────────────────┐        ▼
...
//! - Legalize
//!   - Convert AST to IR (intermediate representation) to ready the following
//!     code generation
//!   - [ir] module defines IR structs, and they implements [ir::Legalize] trait
//!     for legalizing from AST
//! - Code Generation
...
//!
//! This short section shows basic concept of EXPRESS schemas and exchange
//! structures for developers new to STEP ecosystem.
//!
...
//! Entity is a collection of primitive types or other entities similar to
//! `struct` in Rust.
//!
//! Actual data will be stored in another data format called "exchange
//! structure" or "STEP file", standardized as [ISO-10303-21](https://www.iso.org/standard/63141.html).
...
//! ```
//!
//! This describes three instances, `#1` of type `A`, and `#2` and `#3` of type
//! `B`. Data section itself is independent from any schema.
//! The grammar of exchange structure allows any number of fields, e.g. `A(1.0,
//! 2.0, 3.0)`. What data must be contained in the data section is specified in
//! its header section:
//!
//! ```text
//! HEADER;
