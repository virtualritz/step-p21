...
pub type RawParseResult<'a, RawOutput> =
    IResult<&'a str, RawOutput, VerboseError<&'a str>>;
...
impl<'a, P, O1, O2, F>
    nom::Parser<&'a str, (O2, Vec<Remark>), VerboseError<&'a str>>
    for Map<'a, P, O1, O2, F>
where
    P: EsprParser<'a, O1>,
    F: Fn(O1) -> O2,
{
    fn parse(&mut self, input: &'a str) -> ParseResult<'a, O2> {
        let (input, (out, remarks)) =
            nom::Parser::parse(&mut self.parser, input)?;
        let out = (self.f)(out);
...
impl<'a, Output, T> EsprParser<'a, Output> for T where
    T: nom::Parser<&'a str, (Output, Vec<Remark>), VerboseError<&'a str>>
        + Clone
{
}
...
///
/// Be sure that `Vec::new` does not allocates memory until any member will be
/// pushed. <https://doc.rust-lang.org/std/vec/struct.Vec.html#method.new>
    ...
    F: EsprParser<'a, O>,
{
    move |input| -> ParseResult<'a, Option<O>> {
        let (input, value) =
            nom::Parser::parse(&mut nom::combinator::opt(f.clone()), input)?;
...
pub fn not<'a>(
    f: impl EsprParser<'a, &'a str>,
) -> impl EsprParser<'a, &'a str> {
    move |input: &'a str| {
...
pub fn separated<'a, O>(
    ...
    use nom::Parser;
    move |input| {
        let comma_with_remark = nom::sequence::tuple((
            spaces_or_remarks,
            char(c),
            spaces_or_remarks,
        ))
...
pub fn semicolon_separated<'a, O>(
    f: impl EsprParser<'a, O>,
) -> impl EsprParser<'a, Vec<O>> {
    separated(';', f)
}
...
mod tests {
    ...
    fn comma_separated() {
        let (res, (values, remarks)) =
            super::comma_separated(super::remarked(basis::digit))
                .parse("1, (* 2, *) 3, 4")
                .finish()
                .unwrap();
        assert_eq!(res, "");
