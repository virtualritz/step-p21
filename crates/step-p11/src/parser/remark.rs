...
use nom::{
    Parser, branch::alt, bytes::complete::*, character::complete::*,
    combinator::opt, multi::*, sequence::*,
};

fn begin(input: &str) -> RawParseResult<'_, ()> {
    tag("(*").map(|_| ()).parse(input)
}

fn end(input: &str) -> RawParseResult<'_, String> {
...
fn middle_star(input: &str) -> RawParseResult<'_, String> {
...
/// Quoted string like `\`*)\``
fn quoted(input: &str) -> RawParseResult<'_, String> {
    tuple((char('`'), many0(none_of("`")), char('`')))
        .map(|(_quote_start, chars, _quote_end)| {
...
fn non_quoted(input: &str) -> RawParseResult<'_, String> {
    many1(none_of("`*"))
...
pub fn tail_remark(input: &str) -> RawParseResult<'_, Remark> {
    tuple((
        tag("--"),
        space0,
        opt(remark_tag),
...
///
/// `remark_ref` is replaced by `simple_id` because it should be handled by
/// following ir analysis phase.
pub fn remark_tag(input: &str) -> RawParseResult<'_, Vec<String>> {
    delimited(char('"'), separated_list1(char('.'), simple_id), char('"'))
        .parse(input)
}
...
mod tests {
    ...
    fn quoted_end() {
        let (res, stars) =
            super::embedded_remark("(* `*)` should be ignored *)")
                .finish()
                .unwrap();
        assert_eq!(res, "");
    ...
    fn simple() {
        let (res, stars) =
            super::embedded_remark("(* aaa *)").finish().unwrap();
        assert_eq!(res, "");
    ...
    fn stars() {
        ...
        let (res, stars) =
            super::embedded_remark("(* *** *)").finish().unwrap();
        assert_eq!(res, "");
        assert_eq!(stars.remark, "***");
    ...
    fn middle_stars() {
        let (res, stars) =
            super::embedded_remark("(* a * b *)").finish().unwrap();
        assert_eq!(res, "");
    ...
    fn remark_tag() {
        let (res, tag) =
            super::remark_tag(r#""some.name.space""#).finish().unwrap();
        assert_eq!(res, "");
        assert_eq!(
            tag,
