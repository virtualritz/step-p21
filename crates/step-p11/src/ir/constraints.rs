...
impl ConstraintExpr {
    ...
    pub fn is_in(&self, path: &Path) -> bool {
        ...
        match self {
            Reference(p) => p == path,
            AndOr(exprs) | And(exprs) | OneOf(exprs) => {
                exprs.iter().any(|expr| expr.is_in(path))
            }
        }
    }
    ...
    pub fn as_instantiables(
        &self,
        ns: &Namespace,
    ) -> Result<Vec<Vec<Path>>, SemanticError> {
...
pub struct Constraints {
    ...
    /// but possible subtypes cannot be determined from local description in
    /// EXPRESS.
...
pub fn gather_constraint_expr(
        ...
        for entity in &schema.entities {
            match &entity.constraint {
                Some(ast::Constraint::SuperTypeRule(expr)) => {
                    ...
                    // This insert must be first time unless same ENTITY
                    // declaration exists
                    if result.is_some() {
                        return Err(SemanticError::DuplicatedDeclaration(
                            Path::entity(&scope, &entity.name),
                        ));
                    }
                }
    ...
    //
    // The step d) is done before c) because c) have to look up all
    // `SUBTYPE_CONSTRAINT`. New `SUBTYPE_CONSTRAINT` introduced in the step
    // c) will be merged while c).
    //
    for schema in &st.schemas {
        let scope = root.schema(&schema.name);
    ...
    //
    // c-1) Thus, we first read every ENTITY to gather sub- to super-type
    // dependencies,
    let mut super_to_sub: HashMap<
        Path,      /* super */
        Vec<Path>, /* sub */
    > = HashMap::new();
    for schema in &st.schemas {
    ...
    for (sup, subs) in super_to_sub {
        match exprs.entry(sup) {
            ...
            Entry::Vacant(e) => {
                ...
                let subs =
                    subs.into_iter().map(ConstraintExpr::Reference).collect();
