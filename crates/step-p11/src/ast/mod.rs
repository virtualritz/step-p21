    ...
    ($component:ty, $parser:path) => {
        impl $crate::ast::Component for $component {
            fn parse(
                ...
                (Self, Vec<$crate::ast::Remark>),
                $crate::ast::TokenizeFailed,
            > {
                use nom::Finish;
                let input = input.trim();
                let (_input, parsed) =
                    $parser(input).finish().map_err(|err| {
                        $crate::ast::TokenizeFailed::new(input, err)
                    })?;
                Ok(parsed)
            }
        }
...
impl SyntaxTree {
    pub fn parse(input: &str) -> Result<Self, nom::error::VerboseError<&str>> {
        let (residual, (schemas, remarks)) =
            tuple((spaces, many1(schema_decl), spaces))
