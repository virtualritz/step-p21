...
//! Design
//! -------
//! - [step_p11::codegen::rust](../step_p11/codegen/rust/index.html) generates
//!   Rust code with proc-macros defined in this crate.
//! - This crate does not depends on step_p11 explicitly.
...
/// use step_p21_derive::{Holder, TableInit, as_holder};
///
...
/// Generates `Holder` struct and related implementation for each `ENTITY`
/// struct
///
/// `#[holder]` attribute
/// ---------------------
    ...
    match &ast.data {
        syn::Data::Struct(st) => match st.fields {
            ...
            syn::Fields::Unnamed(_) => {
                type_decl::derive_holder(ident, st, &attr)
            }
            syn::Fields::Unit => {
                abort_call_site!("Unit struct is not supported.")
            }
        },
...
mod snapshot_tests {
    ...
    fn derive_holder_enum() {
        ...
        let out = step_p11::codegen::rust::rustfmt(tt.to_string());
        ...
        impl ::step_p21::tables::IntoOwned for S1Holder {
            type Owned = S1;
            type Table = Table;
            ...
            fn into_owned(
                self,
                table: &Self::Table,
            ) -> ::step_p21::error::Result<Self::Owned> {
                Ok(match self {
        ...
        impl ::step_p21::tables::Holder for S1Holder {
            fn name() -> &'static str {
                "S1"
            }
        ...
        impl<'de> ::step_p21::serde::de::Deserialize<'de> for S1Holder {
            fn deserialize<D>(deserializer: D) -> ::std::result::Result<Self, D::Error>
            where
                D: ::step_p21::serde::de::Deserializer<'de>,
            {
        ...
        impl<'de> ::step_p21::serde::de::Visitor<'de> for S1HolderVisitor {
            type Value = S1Holder;
            ...
            where
                A: ::step_p21::serde::de::MapAccess<'de>,
            {
                let key: String = map
                    .next_key()?
                    .expect("Empty map cannot be accepted as step_p21 Holder");
                match key.as_str() {
                    "A" => {
                    ...
                    _ => {
                        use step_p21::serde::de::{Error, Unexpected};
                        return Err(A::Error::invalid_value(
                            Unexpected::Other(&key),
                            &self,
                        ));
                    }
                }
        ...
        impl ::step_p21::tables::WithVisitor for S1Holder {
        ...
        impl ::step_p21::tables::EntityTable<S1Holder> for Table {
            fn get_owned(&self, entity_id: u64) -> ::step_p21::error::Result<S1> {
                if let Ok(owned) = ::step_p21::tables::EntityTable::<AHolder>::get_owned(
                    self, entity_id,
                ) {
                    return Ok(S1::A(Box::new(owned.into())));
                }
                if let Ok(owned) = ::step_p21::tables::EntityTable::<BHolder>::get_owned(
                    self, entity_id,
                ) {
                    return Ok(S1::B(Box::new(owned.into())));
                }
                Err(::step_p21::error::Error::UnknownEntity(entity_id))
            }

            fn owned_iter<'table>(
                &'table self,
            ) -> Box<dyn Iterator<Item = ::step_p21::error::Result<S1>> + 'table> {
                Box::new(::step_p21::itertools::chain![
                    ::step_p21::tables::EntityTable::<AHolder>::owned_iter(self)
                        .map(|owned| owned.map(|owned| S1::A(Box::new(owned.into())))),
                    ::step_p21::tables::EntityTable::<BHolder>::owned_iter(self)
    ...
    fn derive_holder_enum_any_subsuper() {
        ...
        impl ::step_p21::tables::IntoOwned for BaseAnyHolder {
            type Owned = BaseAny;
            type Table = Tables;
            ...
            fn into_owned(
                ...
                Ok(match self {
                    BaseAnyHolder::Base(sub) => {
                        BaseAny::Base(Box::new(sub.into_owned(table)?))
                    }
                    BaseAnyHolder::Sub(sub) => {
                        BaseAny::Sub(Box::new(sub.into_owned(table)?))
                    }
                })
            }
        }
        impl ::step_p21::tables::Holder for BaseAnyHolder {
            fn name() -> &'static str {
                "BASE_ANY"
            }
        ...
        impl<'de> ::step_p21::serde::de::Deserialize<'de> for BaseAnyHolder {
            fn deserialize<D>(deserializer: D) -> ::std::result::Result<Self, D::Error>
            where
                D: ::step_p21::serde::de::Deserializer<'de>,
            {
        ...
        impl<'de> ::step_p21::serde::de::Visitor<'de> for BaseAnyHolderVisitor {
        ...
        impl ::step_p21::tables::WithVisitor for BaseAnyHolder {
        ...
        impl ::step_p21::tables::EntityTable<BaseAnyHolder> for Tables {
            fn get_owned(&self, entity_id: u64) -> ::step_p21::error::Result<BaseAny> {
                if let Ok(owned) =
                    ::step_p21::tables::EntityTable::<BaseHolder>::get_owned(
                        self, entity_id,
                    )
                {
                ...
                if let Ok(owned) =
                    ::step_p21::tables::EntityTable::<SubAnyHolder>::get_owned(
                        self, entity_id,
                    )
                {
            ...
            fn owned_iter<'table>(
                &'table self,
            ) -> Box<dyn Iterator<Item = ::step_p21::error::Result<BaseAny>> + 'table>
            {
                Box::new(::step_p21::itertools::chain![
                    ::step_p21::tables::EntityTable::<BaseHolder>::owned_iter(self)
                        .map(|owned| owned
                            .map(|owned| BaseAny::Base(Box::new(owned.into())))),
                    ::step_p21::tables::EntityTable::<SubAnyHolder>::owned_iter(self)
                        .map(|owned| owned
                            .map(|owned| BaseAny::Sub(Box::new(owned.into()))))
                ])
            }
        }
        "#);
    ...
    fn skip_unrelated_attributes() {
        ...
        pub struct Sub1Holder {
            pub base: ::step_p21::tables::PlaceHolder<BaseHolder>,
            pub y1: f64,
        }
        #[automatically_derived]
        impl ::step_p21::tables::IntoOwned for Sub1Holder {
            type Owned = Sub1;
            type Table = Tables;
        ...
        #[automatically_derived]
        impl ::step_p21::tables::Holder for Sub1Holder {
            fn name() -> &'static str {
                "SUB_1"
            }
        ...
        #[automatically_derived]
        impl ::step_p21::tables::EntityTable<Sub1Holder> for Tables {
            fn get_owned(&self, entity_id: u64) -> ::step_p21::error::Result<Sub1> {
                ::step_p21::tables::get_owned(self, &self.sub1, entity_id)
            }

            fn owned_iter<'table>(
                &'table self,
            ) -> Box<dyn Iterator<Item = ::step_p21::error::Result<Sub1>> + 'table>
            {
                ::step_p21::tables::owned_iter(self, &self.sub1)
            }
        }
        #[doc(hidden)]
        pub struct Sub1HolderVisitor;
        #[automatically_derived]
        impl<'de> ::step_p21::serde::de::Visitor<'de> for Sub1HolderVisitor {
            ...
            where
                A: ::step_p21::serde::de::SeqAccess<'de>,
            {
                if let Some(size) = seq.size_hint() {
                    if size != 2usize {
                        use step_p21::serde::de::Error;
        ...
        #[automatically_derived]
        impl<'de> ::step_p21::serde::de::Deserialize<'de> for Sub1Holder {
            fn deserialize<D>(deserializer: D) -> ::std::result::Result<Self, D::Error>
            where
                D: ::step_p21::serde::de::Deserializer<'de>,
            {
        ...
        #[automatically_derived]
        impl ::step_p21::tables::WithVisitor for Sub1Holder {
