...
impl Input {
    ...
    fn impl_holder(&self) -> TokenStream2 {
        ...
        quote! {
            ...
            impl #step_p21::tables::Holder for #holder_ident {
                fn name() -> &'static str {
                    #name
                }
                fn attr_len() -> usize {
        ...
        quote! {
            ...
            impl<'de> #serde::de::Visitor<'de> for #holder_visitor_ident {
                    ...
                    A: #serde::de::MapAccess<'de>,
                {
                    let key: String = map
                        .next_key()?
                        .expect("Empty map cannot be accepted as step_p21 Holder"); // this must be a bug, not runtime error
                    match key.as_str() {
                        #(
                        #variant_names => {
            ...
            impl #step_p21::tables::WithVisitor for #holder_ident {
    ...
    fn impl_entity_table(&self) -> TokenStream2 {
        ...
        quote! {
            impl #step_p21::tables::EntityTable<#holder_ident> for #table {
                fn get_owned(&self, entity_id: u64) -> #step_p21::error::Result<#ident> {
                    #(
                    if let Ok(owned) = #step_p21::tables::EntityTable::<#holders>::get_owned(self, entity_id) {
                        return Ok(#ident::#vars(#exprs));
                    }
                    )*
                    Err(#step_p21::error::Error::UnknownEntity(entity_id))
                }
                fn owned_iter<'table>(&'table self) -> Box<dyn Iterator<Item = #step_p21::error::Result<#ident>> + 'table> {
                    Box::new(#itertools::chain![
                        #(
                        #step_p21::tables::EntityTable::<#holders>::owned_iter(self)
