...
///
/// ```
/// use step_p21_derive::{as_holder, Holder, TableInit};
...
mod snapshot_tests {
    ...
    fn derive_holder_enum() {
        ...
        impl ::step_p21::tables::IntoOwned for S1Holder {
            type Owned = S1;
            type Table = Table;
            fn into_owned(self, table: &Self::Table) -> ::step_p21::error::Result<Self::Owned> {
                Ok(match self {
        ...
        impl ::step_p21::tables::EntityTable<S1Holder> for Table {
            fn get_owned(&self, entity_id: u64) -> ::step_p21::error::Result<S1> {
                if let Ok(owned) = ::step_p21::tables::EntityTable::<AHolder>::get_owned(self, entity_id) {
                    return Ok(S1::A(Box::new(owned.into())));
                }
                if let Ok(owned) = ::step_p21::tables::EntityTable::<BHolder>::get_owned(self, entity_id) {
    ...
    fn derive_holder_enum_any_subsuper() {
        ...
        impl ::step_p21::tables::EntityTable<BaseAnyHolder> for Tables {
            fn get_owned(&self, entity_id: u64) -> ::step_p21::error::Result<BaseAny> {
                if let Ok(owned) = ::step_p21::tables::EntityTable::<BaseHolder>::get_owned(self, entity_id)
                {
                    return Ok(BaseAny::Base(Box::new(owned.into())));
                }
                if let Ok(owned) =
                    ::step_p21::tables::EntityTable::<SubAnyHolder>::get_owned(self, entity_id)
                {
