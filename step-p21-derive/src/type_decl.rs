...
pub fn def_holder(ident: &syn::Ident, st: &syn::DataStruct) -> TokenStream2 {
    ...
    quote! {
        ...
        pub struct #holder_ident(#(pub #holder_types),*);
    }
}
...
pub fn impl_holder(
    ...
    quote! {
        ...
        impl #step_p21::tables::IntoOwned for #holder_ident {
            type Table = #table;
            type Owned = #ident;
            fn into_owned(self, #table_arg: &Self::Table) -> #step_p21::error::Result<Self::Owned> {
                Ok(#ident ( #(#into_owned),* ))
            }
        }
        #[automatically_derived]
    ...
    quote! {
        ...
        impl #step_p21::tables::EntityTable<#holder_ident> for #table {
            fn get_owned(&self, entity_id: u64) -> #step_p21::error::Result<#ident> {
                #step_p21::tables::get_owned(self, &self.#field, entity_id)
            }
            fn owned_iter<'table>(&'table self) -> Box<dyn Iterator<Item = #step_p21::error::Result<#ident>> + 'table> {
                #step_p21::tables::owned_iter(self, &self.#field)
            }
        }
    }
}
        ...
        for (i, field) in st.fields.iter().enumerate() {
            ...
            if place_holder {
                match &ft {
                    FieldType::Path(_) => {
                        into_owned.push(
                            quote! { self.#index.into_owned(#table_arg)? },
                        );
                    }
